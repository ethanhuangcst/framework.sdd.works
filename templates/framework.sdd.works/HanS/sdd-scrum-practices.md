...
表列固定为：`分类`、`PBI 代码`、`标题`、`描述`、`验收条件`、`关联`、`Sprint`、`状态`。`分类`是一个词。`PBI 代码`为「分类-两位序号」，例如 `Spec-01`，并承载行锚点。同一分类的行排在一起。已经单独成行的文件或技能不再另设父项行。技能文件夹名以 `sdd-` 开头。
...
- 回溯引用以 PBI 代码和条目名为主。行锚点写在 PBI 代码上。
