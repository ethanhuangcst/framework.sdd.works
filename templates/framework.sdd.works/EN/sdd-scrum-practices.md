...
Columns are fixed: `Category`, `PBI Code`, `PBI`, `Description`, `Acceptance criteria`, `Related`, `Sprint`, `Status`. `Category` is one word. `PBI Code` is that word plus a two-digit number, such as `Spec-01`, and it carries the row anchor. Rows with the same category stay together. A file or skill that already has its own row is not also a parent row. Skill folders start with `sdd-`.
